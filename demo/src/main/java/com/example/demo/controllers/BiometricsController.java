...
public class BiometricsController {
    ...
    public ResponseEntity<Biometrics> createBiometrics(@PathVariable Integer userId, @RequestBody Biometrics biometrics, Principal principal) {
        String brand = getRequestingUserPartner(principal);
    ...
    @PutMapping("/{id}")
    @PreAuthorize("hasAnyRole('ADMIN', 'CLIENT')")
    public ResponseEntity<Biometrics> updateBiometrics(@PathVariable Integer id, @RequestBody Biometrics biometrics, Principal principal) {
        String brand = getRequestingUserPartner(principal);
