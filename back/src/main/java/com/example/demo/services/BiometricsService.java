...
public class BiometricsService {
    ...
    public Biometrics updateBiometrics(Integer id, Biometrics details, String requestingUserPartnerBrand) {
        ...
        if (requestingUserPartnerBrand != null
                && !requestingUserPartnerBrand.equals(biometrics.getUser().getPartnerBrand())) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "Access denied");
        }

        if (details.getWeightKg() != null)
            biometrics.setWeightKg(details.getWeightKg());
        if (details.getHeightCm() != null)
            biometrics.setHeightCm(details.getHeightCm());
        if (details.getHeartRate() != null)
            biometrics.setHeartRate(details.getHeartRate());
        if (details.getMeasuredAt() != null)
            biometrics.setMeasuredAt(details.getMeasuredAt());
